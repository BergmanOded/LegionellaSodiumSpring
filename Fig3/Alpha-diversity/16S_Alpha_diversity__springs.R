...
metadata <- read.table(file = "/home/oded/Documents/post/articles/springs/NGS/alpha-diversity/metadata.tsv", sep = "\t", header = T, row.names = 1)
str(metadata)
...
# diversity boxplot spring cluster----
...
p.water.stat.cluster.shannon.text.col <- p.water.cluster.shannon.text.col + stat_pvalue_manual(post.hoc.water.cluster.shannon, label = "p.adj.signif", inherit.aes = FALSE, hide.ns=TRUE, tip.length = 0.002, y.position = c(7, 8, 9, 10))
...
p.water.stat.cluster.faith_pd.text.col <- p.water.cluster.faith_pd.text.col + stat_pvalue_manual(post.hoc.water.cluster.faith_pd, label = "p.adj.signif", inherit.aes = FALSE, hide.ns=TRUE, tip.length = 0.002, y.position = c(8.5, 9.5, 10.5, 11.5, 12.5))
...
p.water.cluster.OTU <- ggplot(metadata_water, aes(x=cluster, y=observed_OTUs, fill = cluster)) +   
  geom_boxplot() +
  theme_classic() + 
  scale_x_discrete(labels= my.labels.water) + scale_y_continuous(limits=c(0, 150)) + 
  theme(legend.position = "none") + labs(x=NULL, y=expression(paste("Observed ASVs")))
...
p.water.stat.cluster.OTU.text.col <- p.water.cluster.OTU.text.col + stat_pvalue_manual(post.hoc.water.cluster.OTU, label = "p.adj.signif", inherit.aes = FALSE, hide.ns=TRUE, tip.length = 0.002, y.position = c(100, 110, 120, 130, 140))
...
p.biofilm.stat.cluster.faith_pd.text.col <- p.biofilm.cluster.faith_pd.text.col + stat_pvalue_manual(post.hoc.biofilm.cluster.faith_pd, label = "p.adj.signif", inherit.aes = FALSE, hide.ns=TRUE, tip.length = 0.002, y.position = c(10, 11.5))
...
p.biofilm.cluster.OTU <- ggplot(metadata_biofilm, aes(x=cluster, y=observed_OTUs, fill = cluster)) +   
  geom_boxplot() +
  theme_classic() + 
  scale_x_discrete(labels= my.labels.biofilm) + scale_y_continuous(limits=c(0, 150)) + 
  theme(legend.position = "none") + labs(x=NULL, y=expression(paste("Observed ASVs")))
...
p.water.date.faith_pd <- ggplot(metadata_water, aes(x=collection_date, y=faith_pd)) +   
  geom_boxplot() +
  theme_classic() + 
  scale_x_discrete(name ="Collection date", labels= my.labels.date) + scale_y_continuous(limits=c(0, 12)) + 
...
p.water.date.OTU <- ggplot(metadata_water, aes(x=collection_date, y=observed_OTUs)) +   
  geom_boxplot() +
  theme_classic() + 
  scale_x_discrete(labels= my.labels.date) + scale_y_continuous(limits=c(0, 150)) + 
  theme(legend.position = "none") + labs(x=NULL, y=expression(paste("Observed ASVs"))) + theme(
    axis.title.y = element_text(color="black", size=8),
    axis.text.x = element_text(color="black", size=8, vjust = -1),
    axis.text.y = element_text(color="black", size=8),
    legend.title = element_text(color = "black", size=8, face ="bold"),
...
#compile plot
    ...
    water_cluster_alpha_diversity
    
    biofilm_cluster_alpha_diversity <- cowplot::plot_grid(p.biofilm.stat.cluster.shannon.text.col, p.biofilm.stat.cluster.faith_pd.text.col, p.biofilm.stat.cluster.evenness.text.col, p.biofilm.stat.cluster.OTU.text.col, align = "v", ncol = 1, rel_heights = c(0.225, 0.225, 0.225, 0.325))
    biofilm_cluster_alpha_diversity
    ...
    water_biofilm_cluster_alpha_diversity
    ...
    pdf("/home/oded/Documents/post/articles/springs/final R code/NGS/16S/alpha_diversity/alpha_diversity_spring_collection.pdf",  width = 20, height = 12, family="ArialMT")
    water_biofilm_cluster_date_alpha_diversity
    dev.off()
    ...
    p.water.biofilm.env.shannon <- ggplot(metadata, aes(x=environment, y=shannon_entropy, fill = environment)) +   
      geom_boxplot() +
      theme_classic() + 
      scale_x_discrete(labels= my.labels.env) + scale_y_continuous(limits=c(0, 10)) + 
    ...
    # match the colours to those of the PCA plot  
    p.water.biofilm.env.shannon.col <- p.water.biofilm.env.shannon + scale_fill_manual(values = c("#7d80d8", "#6b440b"))
    p.water.biofilm.env.shannon.col
    #faith_pd
    
    p.water.biofilm.env.faith <- ggplot(metadata, aes(x=environment, y=faith_pd, fill = environment)) +   
      geom_boxplot() +
      theme_classic() + 
      scale_x_discrete(labels= my.labels.env) + scale_y_continuous(limits=c(0, 12)) + 
    ...
    # match the colours to those of the PCA plot  
    p.water.biofilm.env.faith.col <- p.water.biofilm.env.faith + scale_fill_manual(values = c("#7d80d8", "#6b440b"))
    p.water.biofilm.env.faith.col
    #evenness
    
    p.water.biofilm.env.evenness <- ggplot(metadata, aes(x=environment, y=pielou_evenness, fill = environment)) +   
      geom_boxplot() +
      theme_classic() + 
      scale_x_discrete(labels= my.labels.env) + scale_y_continuous(limits=c(0, 2)) + 
    ...
    # match the colours to those of the PCA plot  
    p.water.biofilm.env.evenness.col <- p.water.biofilm.env.evenness + scale_fill_manual(values = c("#7d80d8", "#6b440b"))
    ...
    p.water.biofilm.env.OTU <- ggplot(metadata, aes(x=environment, y=observed_OTUs, fill = environment)) +   
      geom_boxplot() +
      theme_classic() + 
      scale_x_discrete(labels= my.labels.env) + scale_y_continuous(limits=c(0, 150)) + 
    ...
    # match the colours to those of the PCA plot  
    p.water.biofilm.env.OTU.col <- p.water.biofilm.env.OTU + scale_fill_manual(values = c("#7d80d8", "#6b440b"))
    
    
    environment_date_alpha_diversity <- cowplot::plot_grid(p.water.biofilm.env.shannon.col + theme(legend.position="none"), 
    ...
    environment_date_alpha_diversity
    
    legend_b <- get_legend( environment_date_alpha_diversity + 
    ...
    png("/home/oded/Documents/post/articles/springs/NGS/alpha-diversity/environment_alpha.png",  width = 14800, height = 9000, res = 1200)
    plot_grid(water_biofilm_date_alpha_diversity, legend_b, ncol = 1, rel_heights = c(1, .1))
    dev.off()