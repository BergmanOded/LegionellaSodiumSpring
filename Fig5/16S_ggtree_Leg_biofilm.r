...
# there is more information on adding tree information here: http://yulab-smu.top/treedata-book/chapter2.html#accesor-tidytree
...
feature_table <- read.table(file = "/home/oded/Documents/post/articles/springs/tree/for_tree/percent/biofilm/0.5/last_5/feature-table.tsv", sep = ",", header = T, row.names = 1, 
                        skip = 1, comment.char = "")
str(feature_table)
# In ggtree anf phyloseq, the trees are presentred in a way that the group is presentred for
# every sample. and as each ASV ca appear in many samples, it will be annotated with
...
# Then I binded them
# and finaly created a new df with only the sumed spring cluster column
...
phyloseq.rooted.tree <- ggtree(phyloseq.rooted.tree) +  geom_tippoint(aes(x=x+1.1*hjust, color=Spring_Cluster), size = 2, position="identity") +
  scale_color_manual(values = c("#D55E00","gray","#56B4E9", "#CC79A7"), na.translate = F) + xlim(-0.00005, 0.5) +
  geom_tiplab(align=TRUE, linesize=0.25, offset = 0.02)+
  geom_point2(aes(subset = !isTip, shape  = label), size=1, show_guide = FALSE)+
  scale_shape_manual("Bootstrap Values", #legend name
                     values = c(32, 16),
                     na.translate = F,#remove na
...
phyloseq.rooted.tree


pdf("/home/oded/Documents/post/articles/springs/tree/for_tree/percent/biofilm/0.5/last_5/tree_ML_Leg_springs_biofilm_0.5reads_regular.pdf", width = 10, height = 15, family="ArialMT")
phyloseq.rooted.tree
dev.off()
